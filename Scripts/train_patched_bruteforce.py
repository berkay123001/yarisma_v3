...
def apply_patch_feature(df):
    print("Applying Patch Feature (Risk Flags)...")
    # Derived from Error Analysis
    # Rule 1: credit_score <= 699.50 AND debt_to_income_ratio <= 0.18
    # Rule 2: credit_score > 699.50 AND debt_to_income_ratio > 0.15 AND employment_status <= 2.50
    
    # We create a categorical 'risk_flag'
    # 0: Normal
    # 1: High Risk Pattern 1 (Low Score, Low DTI - Deceptive)
    # 2: High Risk Pattern 2 (High Score, High DTI, Low Emp - Deceptive)
    
    conditions = [
        (df['credit_score'] <= 700) & (df['debt_to_income_ratio'] <= 0.18),
        (df['credit_score'] > 700) & (df['debt_to_income_ratio'] > 0.15) & (df['employment_status'] <= 3)
    ]
    choices = [1, 2]
    
    df['risk_flag'] = np.select(conditions, choices, default=0)
    
    # Also add interaction with the flag
    df['risk_flag_x_dti'] = df['risk_flag'] * df['debt_to_income_ratio']
...
def main():
    ...
    train = apply_patch_feature(train)
    test = apply_patch_feature(test)
    ...
    print("\n--- Training Patched Brute-Force LightGBM ---")
    ...
    print(f"\nFinal Patched OOF AUC: {auc_score:.5f}")
    
    # Save Submission
    os.makedirs('Submissions', exist_ok=True)
    sub_df = pd.DataFrame({'id': test_ids, TARGET_COL: test_preds})
    sub_df.to_csv('Submissions/submission_patched_bruteforce.csv', index=False)
    print("Saved Submission to Submissions/submission_patched_bruteforce.csv")
    
    # Save OOF
    os.makedirs('Models', exist_ok=True)
    pd.DataFrame({TARGET_COL: oof_preds}).to_csv('Models/oof_patched_bruteforce.csv', index=False)
